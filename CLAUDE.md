...
It consumes `app/shared` as a **vendored copy** (`vendor/shared/` over there,
regenerated by `npm run sync:shared`), NOT a live symlink — EAS only uploads
that repo's own git archive, so anything outside it is absent at build time.
A breaking change to the shared package still breaks the phone app, just at
its next sync rather than immediately. **After editing `app/shared`, the
Android repo needs `npm run sync:shared` + a commit**; its `npm test`
content-hashes both copies and fails if they have drifted.
